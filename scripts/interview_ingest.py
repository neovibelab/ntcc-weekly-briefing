...
def classify(title: str, text: str, media: str) -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            "제품·서비스·회사·플랫폼 이름은 **원문 표기 그대로** 둔다(Lyria·Suno·Gemini·TuneCore를 음역하지 않는다). 일본어·중국어 개념어는 국내에서 통용되는 표기를 쓴다(推し活는 오시카츠, 谷子经济는 구쯔경제, 微短剧는 마이크로 숏드라마). 같은 대상을 매번 같은 표기로 쓴다 - 표기가 흔들리면 같은 사건이 안 묶인다.\n"
